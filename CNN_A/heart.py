...
class Dataset(object):
    ...
    def _read_all_dicom_images(self):
        ...
        if np.sum(idx)>0:
            print("WARN, some slices has non parallel direction, remove it!",self.slices_ver[idx]);
        ...
        img_shape = c.most_common()[0][0];#take most occured image size (may not good??, should crop it?)
